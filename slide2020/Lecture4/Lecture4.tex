...
\item[LA] Parallel Basic Linear Algebra Subprograms
	\begin{itemize}
		\item \texttt{ScaLAPACK}: \url{www.netlib.org/scalapack}
		\item \alert<3>{\texttt{PSBLAS}}: \url{people.uniroma2.it/salvatore.filippone/psblas}
		\item \texttt{PETSc}: \url{www.mcs.anl.gov/petsc}
	\end{itemize}
\item[Solvers] Solvers for Linear and Nonlinear equations
	\begin{itemize}
		\item \texttt{Hypre}: \url{computing.llnl.gov/projects/hypre-scalable-linear-solvers-multigrid-methods}
		\item \alert<3>{\texttt{MLD2P4}}: \url{github.com/sfilippone/mld2p4-2}
		\item \texttt{MUMPS}: \url{mumps.enseeiht.fr/}
		\item \texttt{Trilinos}: \url{trilinos.github.io/}
		\item \texttt{SUNDIALS}: \url{computing.llnl.gov/projects/sundials}
	\end{itemize}
\item[Repos]
\begin{itemize}
	\item \url{software.llnl.gov}
	\item \url{developer.nvidia.com/gpu-accelerated-libraries}
	\item \url{en.wikipedia.org/wiki/List_of_numerical_libraries}
\end{itemize}
\end{itemize}
}
\end{frame}
